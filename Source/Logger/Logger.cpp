...
void Logger::WriteToLog(LogLevel level, const char* file, int line, const char* func, const char* format, va_list va)
{
	...
	for (auto p = file; *p != '\0'; p++)
		if (*p == '\\' || *p == '/')
			file = p + 1;
	int len1 = snprintf(t_LogBuffer, MAX_LOG_FORMAT_LENGTH, "%s %d %s ", GetLocalDateTimeWithMilliSecond().c_str(), GetThreadID(), s_LogLevelName[level].c_str());
...
void Logger::WriteToConsole(LogLevel level, const char* formatStr, va_list va)
{
	...
	int len = snprintf(logString, MAX_LOG_FORMAT_LENGTH, "ThreadID[%05d] ", GetThreadID());
...
unsigned int Logger::GetThreadID()
{
